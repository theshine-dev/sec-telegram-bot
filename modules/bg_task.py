...
async def discover_new_filings():
    ...
    tickers = db_manager.get_all_subscribed_tickers()
    ...
    for ticker in tickers:
        ...
        last_accession_num = db_manager.get_last_accession_number(ticker)
        ...
        if new_filings_to_process:
            ...
            for new_filing in reversed(new_filings_to_process): # 오래된 공시부터 DB 삽입
                ...
                db_manager.update_analysis_queue(FilingInfo(
            ...
            db_manager.update_last_filing_info(FilingInfo(
...
async def process_analysis_queue():
    ...
    for job in jobs:
        ...
        try:
            ...
            db_manager.insert_analysis_archive(job)
            await send_filing_notification_to_users(job)
            db_manager.remove_analysis_queue(job)
            ...
            db_manager.update_analysis_queue(job)


def calc_current_quota_status() -> tuple[int, int]:
    """
    ...
    quota_status = db_manager.get_quota_status()

    current_count = 0
    # DB에 저장된 날짜가 오늘인지 확인
    if quota_status['date'] == today_utc_str:
        # 오늘 날짜: 기존 카운트를 사용
